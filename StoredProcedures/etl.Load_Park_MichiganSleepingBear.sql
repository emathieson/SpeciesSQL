...
/****** Object:  StoredProcedure [etl].[Load_Park_MichiganSleepingBear]    Script Date: 9/7/2023 6:26:36 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE OR ALTER PROCEDURE [etl].[Load_Park_MichiganSleepingBear]


AS
/*
PURPOSE:
Truncates and then loads information into the Park.MichiganSleepingBear table
...
CREATE TABLE #herp_base
	...
	INSERT INTO #herp_base
		...
		SELECT 
			...
			,hps.[Order]
			,hps.Family
			,hps.MichiganSleepingBear
			,hps.DailyActivity
			,hps.YearlyActivity
			,hps.IUCN
			,hps.Habitat
		FROM Species.Herps_current (NOLOCK) hps
		WHERE hps.MichiganSleepingBear IS NOT NULL
...
CREATE TABLE #Fish_base
	...
	INSERT INTO #Fish_base
		...
		SELECT 
			...
			,fsh.[Order]
			,fsh.Family
			,fsh.MichiganSleepingBear
			,fsh.DailyActivity
			,fsh.YearlyActivity
			,fsh.IUCN
			,fsh.Habitat
		FROM Species.Fish_current (NOLOCK) fsh
		WHERE fsh.MichiganSleepingBear IS NOT NULL
...
CREATE TABLE #Mammals_base
	...
	INSERT INTO #Mammals_base
		...
		WHERE mam.MichiganSleepingBear IS NOT NULL
...
CREATE TABLE #Inverts_base
	...
	INSERT INTO #Inverts_base
		...
		SELECT 
			...
			,inv.[Order]
			,inv.Family
			,inv.MichiganSleepingBear
			,inv.DailyActivity
			,inv.YearlyActivity
			,inv.IUCN
			,inv.Habitat
		FROM Species.Inverts_current (NOLOCK) inv
		WHERE inv.MichiganSleepingBear IS NOT NULL
...
CREATE TABLE #Birds_base
	...
	INSERT INTO #Birds_base
		...
		SELECT 
			...
			,brd.[Order]
			,brd.Family
			,brd.MichiganSleepingBear
			,brd.DailyActivity
			,brd.YearlyActivity
			,brd.IUCN
			,brd.Habitat
		FROM Species.Birds_current (NOLOCK) brd
		WHERE brd.MichiganSleepingBear IS NOT NULL
...
TRUNCATE TABLE Park.MichiganSleepingBear
INSERT INTO Park.MichiganSleepingBear
SELECT * FROM #temp_union

--Display table after insert
SELECT * FROM Park.MichiganSleepingBear

...
/****** Object:  StoredProcedure [etl].[Load_Park_MichiganIsleRoyale]    Script Date: 9/7/2023 6:25:46 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE OR ALTER PROCEDURE [etl].[Load_Park_MichiganIsleRoyale]


AS
/*
PURPOSE:
Truncates and then loads information into the Park.MichiganIsleRoyale table
...
CREATE TABLE #herp_base
	...
	INSERT INTO #herp_base
		...
		SELECT 
			...
			,hps.[Order]
			,hps.Family
			,hps.MichiganIsleRoyale
			,hps.DailyActivity
			,hps.YearlyActivity
			,hps.IUCN
			,hps.Habitat
		FROM Species.Herps_current (NOLOCK) hps
		WHERE hps.MichiganIsleRoyale IS NOT NULL
...
CREATE TABLE #Fish_base
	...
	INSERT INTO #Fish_base
		...
		SELECT 
			...
			,fsh.[Order]
			,fsh.Family
			,fsh.MichiganIsleRoyale
			,fsh.DailyActivity
			,fsh.YearlyActivity
			,fsh.IUCN
			,fsh.Habitat
		FROM Species.Fish_current (NOLOCK) fsh
		WHERE fsh.MichiganIsleRoyale IS NOT NULL
...
CREATE TABLE #Mammals_base
	...
	INSERT INTO #Mammals_base
		...
		WHERE mam.MichiganIsleRoyale IS NOT NULL
...
CREATE TABLE #Inverts_base
	...
	INSERT INTO #Inverts_base
		...
		SELECT 
			...
			,inv.[Order]
			,inv.Family
			,inv.MichiganIsleRoyale
			,inv.DailyActivity
			,inv.YearlyActivity
			,inv.IUCN
			,inv.Habitat
		FROM Species.Inverts_current (NOLOCK) inv
		WHERE inv.MichiganIsleRoyale IS NOT NULL
...
CREATE TABLE #Birds_base
	...
	INSERT INTO #Birds_base
		...
		SELECT 
			...
			,brd.[Order]
			,brd.Family
			,brd.MichiganIsleRoyale
			,brd.DailyActivity
			,brd.YearlyActivity
			,brd.IUCN
			,brd.Habitat
		FROM Species.Birds_current (NOLOCK) brd
		WHERE brd.MichiganIsleRoyale IS NOT NULL
...
TRUNCATE TABLE Park.MichiganIsleRoyale
INSERT INTO Park.MichiganIsleRoyale
SELECT * FROM #temp_union

--Display table after insert
SELECT * FROM Park.MichiganIsleRoyale

...
/****** Object:  StoredProcedure [etl].[Load_Park_YellowstoneGrandTeton]    Script Date: 9/7/2023 6:29:18 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE OR ALTER PROCEDURE [etl].[Load_Park_YellowstoneGrandTeton]


AS
/*
PURPOSE:
Truncates and then loads information into the Park.YellowstoneGrandTeton table
...
CREATE TABLE #herp_base
	...
	INSERT INTO #herp_base
		...
		SELECT 
			...
			,hps.[Order]
			,hps.Family
			,hps.YellowstoneGrandTeton
			,hps.DailyActivity
			,hps.YearlyActivity
			,hps.IUCN
			,hps.Habitat
		FROM Species.Herps_current (NOLOCK) hps
		WHERE hps.YellowstoneGrandTeton IS NOT NULL
...
CREATE TABLE #Fish_base
	...
	INSERT INTO #Fish_base
		...
		SELECT 
			...
			,fsh.[Order]
			,fsh.Family
			,fsh.YellowstoneGrandTeton
			,fsh.DailyActivity
			,fsh.YearlyActivity
			,fsh.IUCN
			,fsh.Habitat
		FROM Species.Fish_current (NOLOCK) fsh
		WHERE fsh.YellowstoneGrandTeton IS NOT NULL
...
CREATE TABLE #Mammals_base
	...
	INSERT INTO #Mammals_base
		...
		WHERE mam.YellowstoneGrandTeton IS NOT NULL
...
CREATE TABLE #Inverts_base
	...
	INSERT INTO #Inverts_base
		...
		SELECT 
			...
			,inv.[Order]
			,inv.Family
			,inv.YellowstoneGrandTeton
			,inv.DailyActivity
			,inv.YearlyActivity
			,inv.IUCN
			,inv.Habitat
		FROM Species.Inverts_current (NOLOCK) inv
		WHERE inv.YellowstoneGrandTeton IS NOT NULL
...
CREATE TABLE #Birds_base
	...
	INSERT INTO #Birds_base
		...
		SELECT 
			...
			,brd.[Order]
			,brd.Family
			,brd.YellowstoneGrandTeton
			,brd.DailyActivity
			,brd.YearlyActivity
			,brd.IUCN
			,brd.Habitat
		FROM Species.Birds_current (NOLOCK) brd
		WHERE brd.YellowstoneGrandTeton IS NOT NULL
...
TRUNCATE TABLE Park.YellowstoneGrandTeton
INSERT INTO Park.YellowstoneGrandTeton
SELECT * FROM #temp_union

--Display table after insert
SELECT * FROM Park.YellowstoneGrandTeton
